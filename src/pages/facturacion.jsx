...
function Facturacion() {
  ...
  async function cargarFacturas() {
    ...
    if (idImprimir) {
      ...
      if (factura) {
        await abrirFactura(factura, true);
      }
    }
    ...
    return facturas.reduce(
      (acumulado, factura) => {
        ...
        if (factura.estado === "pagada") {
          acumulado.cobrado += total;
        }

        if (factura.estado === "pendiente") {
          acumulado.pendiente += total;
        }
  ...
  async function guardarDetalleManual() {
    ...
    const { data, error: supabaseError } = await supabase
      .from("facturas")
      .update({
        detalle_concepto: detalleManual.trim(),
        updated_at: new Date().toISOString(),
      })
      .eq("id", facturaAbierta.id)
      .select("*")
      .single();
    ...
    setMensaje("Detalle de la factura guardado correctamente.");
    setGuardando(false);
  }
  ...
  return (
    <section className="panel facturacion-page">
      ...
      {error && (
        <p className="mensaje-error no-imprimir">
          Error: {error}
        </p>
      )}

      {mensaje && (
        <p className="mensaje no-imprimir">
          {mensaje}
        </p>
      )}
      ...
      <div className="facturacion-resumen no-imprimir">
        ...
        <article>
          <span>Cobrado</span>
          <strong>{moneda(resumen.cobrado)}</strong>
        </article>
      ...
      {facturaAbierta && (
        <section
          id="vista-factura"
          className="factura-vista-completa"
        >
          ...
          <div className="factura-cobro-grid no-imprimir">
            <label>
              ...
              <select
                ...
                disabled={guardando}
                onChange={(evento) =>
                  actualizarCobro(facturaAbierta, {
            ...
            <label>
              ...
              <input
                ...
                disabled={
                  guardando ||
                  facturaAbierta.estado !== "pagada"
                }
                onChange={(evento) =>
                  actualizarCobro(facturaAbierta, {
                    fecha_pago: evento.target.value,
                  })
                }
              />
            </label>
          ...
          <div className="factura-detalle-manual no-imprimir">
            <label>
              Detalle concepto factura
              <textarea
                value={detalleManual}
                onChange={(evento) =>
                  setDetalleManual(evento.target.value)
                }
                placeholder="Escribe aquí el concepto que quieres que aparezca en la factura."
                rows="6"
              />
            </label>
      ...
      <div className="facturacion-filtros no-imprimir">
        ...
        <select
          value={filtroPago}
          onChange={(evento) =>
            setFiltroPago(evento.target.value)
          }
        >
  ...
  return (
    <article className="documento-factura-imprimir factura-modelo-cliente">
      ...
      <section className="factura-modelo-bloque">
        ...
        <div className="factura-modelo-subtitulo">
          Datos Cliente
        </div>
