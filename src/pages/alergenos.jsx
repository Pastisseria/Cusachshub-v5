...
export default function Alergenos() {
  const [filas,setFilas]=useState([]); const [busca,setBusca]=useState(""); const [filtro,setFiltro]=useState(""); const [categoria,setCategoria]=useState("pasteleria");
  ...
  useEffect(()=>{cargar();},[]);
  const visibles=useMemo(()=>filas.filter(f=>(f.categoria||"pasteleria")===categoria&&f.producto.toLowerCase().includes(busca.toLowerCase())&&(!filtro||(f.alergenos||[]).includes(filtro)||(f.trazas||[]).includes(filtro))),[filas,busca,filtro,categoria]);
  ...
  async function guardar(){ if(!form.producto.trim())return setMensaje("Escribe el nombre del producto."); setMensaje("Guardando…"); let q; if(editando==="nuevo") q=await supabase.from("higiene_alergenos").insert({...form,producto:form.producto.trim(),categoria,orden:filas.length+1}); else q=await supabase.from("higiene_alergenos").update({...form,producto:form.producto.trim(),updated_at:new Date().toISOString()}).eq("id",editando); if(q.error)return setMensaje("No se ha podido guardar."); setEditando(null);setForm(VACIO);setMensaje("Guardado correctamente.");cargar(); }
  async function eliminar(id){if(!confirm("¿Eliminar este producto de la lista de alérgenos?"))return; const {error}=await supabase.from("higiene_alergenos").update({activo:false,updated_at:new Date().toISOString()}).eq("id",id); if(error)setMensaje("No se ha podido eliminar."); else cargar();}
  return <div className="alerg-page">
    <div className="alerg-head"><div><Link to="/higiene/preparacion-sanidad">← Preparación Sanidad</Link><span className="alerg-kicker">FORMATO 12 · SANIDAD</span><h1>🌾 Lista de alérgenos</h1><p>Productos acabados, alérgenos y posibles trazas de Pastisseria Cusachs.</p></div><div className="alerg-actions"><button onClick={nuevo}>＋ Añadir producto</button><button className="secondary" onClick={()=>window.print()}>🖨 Imprimir / PDF</button></div></div>
    <div className="alerg-tabs no-print"><button className={categoria==="pasteleria"?"active":""} onClick={()=>setCategoria("pasteleria")}>🥐 Pastelería</button><button className={categoria==="comida"?"active":""} onClick={()=>setCategoria("comida")}>🍽️ Comida para llevar</button></div>
    {mensaje&&<div className="alerg-msg">{mensaje}</div>}
    <div className="alerg-tools"><input value={busca} onChange={e=>setBusca(e.target.value)} placeholder="🔎 Buscar producto…"/><select value={filtro} onChange={e=>setFiltro(e.target.value)}><option value="">Todos los alérgenos</option>{ALERGENOS.map(a=><option key={a}>{a}</option>)}</select><strong>{visibles.length} productos · {categoria==="pasteleria"?"Pastelería":"Comida para llevar"}</strong></div>
