...
export default function Incidencias() {
  ...
  useEffect(() => { cargar(); }, []);
  async function guardar(e) { e.preventDefault(); const payload = { ...form, numero_albaran: form.hay_albaran ? form.numero_albaran : null, responsable: form.responsable || form.firma_correccion || form.firma_incidencia }; const { error } = await supabase.from("higiene_incidencias").insert(payload); setMensaje(error ? error.message : "Incidencia guardada correctamente."); if (!error) { setForm({ ...inicial, fecha: hoy(), fecha_correccion: hoy() }); cargar(); } }
  ...
  return <Pagina icono="⚠️" titulo="Registro de incidencias" texto="Versión digital del registro de incidencias y medidas correctoras.">
    <form className="control-form" onSubmit={guardar}>
      ...
      </div><button className="boton-control">Guardar registro de incidencia</button>
