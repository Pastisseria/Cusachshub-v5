...
export default function ControlRecepcion() {
  ...
  async function subirTodos() {
    ...
    for (const archivo of archivos) {
      try {
        ...
        setMensaje(`Se guardaron ${guardados}. Error en ${archivo.name}: ${error.message}`);
        setSubiendo(false);
        await cargar();
        return;
      }
  ...
  async function marcarSelladoExistente(registro) {
    ...
    const proveedor = proveedores.find((item) => item.id === registro.proveedor_id) || encontrarProveedor(registro.proveedor);
    const controlesActuales = registro.controles || {};
    const controles = {
      ...controlesActuales,
      temperatura_estado: controlesActuales.temperatura_estado || "conforme",
      ...Object.fromEntries(CAMPOS_CONTROL.map(([clave]) => [clave, controlesActuales[clave] === "no_conforme" ? "no_conforme" : "conforme"]))
    };
    const { error } = await supabase.from("higiene_control_recepcion").update({
      sello_detectado: true,
      estado_revision: "conforme",
      proveedor_id: proveedor?.id || registro.proveedor_id || null,
      proveedor: proveedor?.nombre || registro.proveedor || null,
      controles,
      revisado_at: new Date().toISOString()
    }).eq("id", registro.id);
    if (error) return setMensaje(`No se pudo marcar como ya sellado: ${error.message}`);
    relecturas.current.add(registro.id);
    setMensaje("Marcado como ya sellado. No se volverá a añadir otro sello.");
    await cargar(false);
  }
  ...
  async function guardarRevision(evento) {
    ...
    try {
      ...
      setSubiendo(false);
      return setMensaje(`No se pudo guardar el control: ${error.message}`);
    }
    setSubiendo(false);
    setEditando(null);
