...
export default function ProduccionInternaPasteleria() {
  ...
  function mover(dias) { const d = new Date(`${fecha}T12:00:00`); d.setDate(d.getDate()+dias); const f=d.toISOString().slice(0,10); if(f>=INICIO&&f<=FIN)setFecha(f); }

  return <div className="pip-page">
    <div className="pip-tools no-print">
      <h1>Producción interna Pastelería</h1>
      <p>Plantilla diaria para imprimir y rellenar manualmente.</p>
      <div className="pip-nav"><button onClick={()=>mover(-1)}>← Día anterior</button><input type="date" min={INICIO} max={FIN} value={fecha} onChange={(e)=>setFecha(e.target.value)}/><button onClick={()=>mover(1)}>Día siguiente →</button><button className="print-btn" onClick={()=>window.print()}>🖨 Imprimir A4</button></div>
    </div>
    ...
    <div className="pip-sheet">
      ...
      <div className="pip-columns">
        {GRUPOS.map((g)=><section key={g.titulo} className="pip-group"><h3>{g.titulo}<span>UNIDADES</span></h3>{g.items.map((item)=><div className="pip-row" key={item}><div>{item}</div><div className="pip-box"></div></div>)}</section>)}
      </div>
