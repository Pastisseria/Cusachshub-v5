...
import "../styles/camareros-estados.css";
...
function estadoPersonal(catering) {
  const necesarios = Number(catering.camareros_necesarios ?? catering.camareros_presupuesto ?? 0);
  const confirmados = Number(catering.camareros_confirmados || 0);
  if (confirmados >= necesarios) return "completo";
  return "pendiente";
}

const numeroNecesarios = (catering) => Number(catering.camareros_necesarios ?? catering.camareros_presupuesto ?? 0);
...
export default function Camareros() {
  ...
  async function cargar() {
    ...
    if (error) setMensaje(`No se pudo cargar el calendario: ${error.message}`);
      ...
      const camarerosPorPresupuesto = (lineas.data || []).reduce((mapa, linea) => {
        mapa[linea.presupuesto_id] = (mapa[linea.presupuesto_id] || 0) + Number(linea.cantidad || 0);
        return mapa;
      }, {});
      setCaterings((eventos.data || []).filter((item) => String(item.estado || "").toLowerCase() !== "cancelado" && camarerosPorPresupuesto[item.presupuesto_id] !== undefined).map((item) => ({ ...item, camareros_presupuesto: camarerosPorPresupuesto[item.presupuesto_id] || 1 })));
      setClientes(personas.data || []);
    }
    setCargando(false);
  }
  useEffect(() => { cargar(); }, []);
  ...
  const totalCamareros = eventosMes.reduce((suma, evento) => suma + numeroNecesarios(evento), 0);
  const renderEvento = (evento, amplio = false) => { const necesarios = numeroNecesarios(evento); const confirmado = estadoPersonal(evento) === "completo"; return <button type="button" key={evento.id} className={`camareros-evento ${estadoPersonal(evento)}`} onClick={() => abrir(evento)} title={`${nombreCliente(evento)} · ${confirmado ? "Confirmado" : "Pendiente de confirmar"}`}><strong>{hora(evento.hora_inicio)} {nombreCliente(evento)}</strong><span>{confirmado ? `🟡 Confirmado · ${evento.camareros_confirmados || 0}/${necesarios}` : `🔵 Pendiente · ${evento.camareros_confirmados || 0}/${necesarios}`}</span>{amplio && <small>{evento.numero_personas || 0} comensales · {evento.direccion || "Sin dirección"}</small>}</button>; };
  ...
  return <main className="camareros-page">
    <header className="camareros-cabecera"><div><span>PERSONAL DE SERVICIO</span><h1>Calendario de camareros</h1><p>Solo aparecen presupuestos que incluyen servicio de camarero.</p></div><div className="camareros-resumen"><b>{totalCamareros}<small>camareros solicitados</small></b><b className={pendientes ? "alerta" : ""}>{pendientes}<small>pendientes de confirmar</small></b></div></header>
    ...
    {cargando ? <p>Cargando calendario…</p> : vista === "mes" ? <section className="camareros-calendario"><div className="camareros-dias">{DIAS.map((dia) => <b key={dia}>{dia}</b>)}</div><div className="camareros-mes">{diasMes(fechaVisible).map((dia) => { const fecha = iso(dia); const eventos = caterings.filter((evento) => evento.fecha === fecha); const total = eventos.reduce((suma, evento) => suma + numeroNecesarios(evento), 0); return <div key={fecha} className={`camareros-dia ${dia.getMonth() !== fechaVisible.getMonth() ? "otro" : ""}`}><span className="camareros-numero">{dia.getDate()}</span>{total > 0 && <span className="camareros-total">👥 {total}</span>}<div>{eventos.slice(0, 3).map((evento) => renderEvento(evento))}{eventos.length > 3 && <small>+ {eventos.length - 3} caterings</small>}</div></div>; })}</div></section> : <section className="camareros-semana">{semana.map((dia) => { const eventos = caterings.filter((evento) => evento.fecha === iso(dia)); const total = eventos.reduce((suma, evento) => suma + numeroNecesarios(evento), 0); return <article key={iso(dia)}><header><b>{DIAS[(dia.getDay() + 6) % 7]}</b><span>{dia.getDate()}/{dia.getMonth() + 1}</span><strong>👥 {total}</strong></header>{eventos.length ? eventos.map((evento) => renderEvento(evento, true)) : <p>Sin servicios de camareros</p>}</article>; })}</section>}
